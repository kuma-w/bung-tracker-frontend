  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h2 className="text-base font-bold text-gray-800">{formatDate(event.event_date)}</h2>
          <p className="text-sm text-gray-400 mt-0.5">1인 {event.amount_per_person.toLocaleString()}원</p>
        </div>
        <span
        ...
        >
          {isFull ? '마감' : `${totalCapacity - totalCount}자리 남음`}
        </span>
      </div>
        ...
        {event.slots.map(slot => (
          <div key={slot.slot_time}>
            <div className="flex justify-between text-xs text-gray-500 mb-1">
              <span className="font-medium text-gray-700">{slot.slot_time}</span>
              <span>
      ...
      <div className="mt-4 pt-3 border-t border-gray-50 flex justify-between items-center text-sm">
        ...
        <span className="text-indigo-500 text-xs font-medium">상세 보기 →</span>
      </div>
    </div>
  )
}
...
export default function EventsPage() {
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-20 text-gray-400">
        ...
        <p className="font-medium">예정된 벙이 없습니다.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-gray-800 mb-5">벙 일정</h1>
